...
export function buildFullConfig(
  ...
  const projectName = answers.projectName || setupConfig.projectName || 'MyProject'
  const teamPriority = answers.teamPriority || setupConfig.teamFocus || 'balanced'
  const stack = answers.stack || setupConfig.stack?.join(',') || 'bun,drizzle,hono'
